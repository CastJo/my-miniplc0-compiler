...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseVariableDeclaration() {
		...
		// <标识符>
			...
			if (!next.has_value() || next.value().GetType() != TokenType::IDENTIFIER)
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNeedIdentifier);
			auto ide = next.value();
			if (isDeclared(ide.GetValueString()))
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrDuplicateDeclaration);
			addUninitializedVariable(ide);//加入到未初始化变量表
		...
		// ';'
			...
			if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
			_vars[ide.GetValueString()] = getIndex(ide.GetValueString());//从未初始化复制到初始化对应位置
			_uninitialized_vars.erase(ide.GetValueString());
			_instructions.emplace_back(Operation::STO, getIndex(ide.GetValueString()));
		}
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		if (!next.has_value()|| next.value().GetType() != TokenType::UNSIGNED_INTEGER)
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrIncompleteExpression);
		out = prefix*std::any_cast<int>(next.value().GetValue());
		return {};
	}
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() {
		...
		bool ini = isInitializedVariable(str);
		next = nextToken();
		if (!next.has_value() || next.value().GetType() != TokenType::EQUAL_SIGN)
		...
		if (!next.has_value() || next.value().GetType() != TokenType::SEMICOLON)
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNoSemicolon);
		if (!ini) {
			_vars[str] = getIndex(str);//从未初始化复制到初始化对应位置
